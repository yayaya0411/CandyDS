...
def plot_obj(data,cols,drops=[],output=False):
    """quick review object data
    data : data
    drop_col : drop object cols that you don't want plot
    ...
    if fig_rows == 1 or cols == 1:
        ...
        for i in range(0,fig_rows):
            for j in range(0,cols):
                try:
                    orders = data_tmp.iloc[:,counter].value_counts().index
                    sns.countplot(x=loop_cols[counter],data=data_tmp,ax=ax[i,j],order= orders)
                    plt.xticks(rotation=45)
                    values(ax[i,j])
                    print(str(i)+','+str(j)+' plot '+ loop_cols[counter])
                    counter += 1
                except IndexError:
...
def plot_int(data,cols,drops=[],bins=10,output=False,kde=False):  #not yet
    """quick review int/float data
    data : data
    bins : automatic grouping of bins
    drop_col : drop int/float cols that you don't want plot
    cols : contral figure cols
    ...
    if output:
        for i in range(len(loop_cols)):
            ...
            fig.savefig(os.path.join(output_folder,'int_'+loop_cols[i]+pic_format))
    fig_rows = math.ceil(data_tmp.shape[1]/cols)
    fig, ax = plt.subplots(fig_rows,cols,figsize=(cols*8, fig_rows*6))
    counter = 0
    print(str(cols)+' x '+str(fig_rows) + ' figure '+str(data_tmp.shape[1]) + ' Variable ' )
    ...
    if fig_rows == 1 or cols == 1:
        for i in range(max(fig_rows,cols)):
            try:
                a = loop_cols[counter]
                sns.distplot(data_tmp[a].dropna(),bins = bins,kde = kde,ax=ax[i])
                plt.xticks(rotation=45)
                print(str(i)+' plot '+ loop_cols[counter])
                counter += 1
            except:
        ...
        for i in range(0,fig_rows):
            for j in range(0,cols):
                try:
                    a = loop_cols[counter]
                    sns.distplot(data_tmp[a].dropna(),bins = bins,kde = kde,ax=ax[i,j])
                    plt.xticks(rotation=45)
                    print(str(i)+','+str(j)+' plot '+ loop_cols[counter])
                    counter += 1
                except IndexError:
